...
int socket_create(char* ip,char* puerto,char tipo_socket, int cant_clientes,t_log* logger){
	...
	//for(p = servinfo; p != NULL; p = p->ai_next) {
		if ((socket_creado = socket(AF_INET, SOCK_STREAM,
				0)) == -1) {
		...
		if(tipo_socket == TIPO_SERVIDOR){
			...
			if (bind(socket_creado, (struct sockaddr *)&socket_info, sizeof(socket_info)) == -1) {//bindeo puerto e ip de la estructura esa
				close(socket_creado);
			...
			if (connect(socket_creado, (struct sockaddr *)&socket_info, sizeof(socket_info)) == -1) {
				close(socket_creado);
				return 0;
			}
		}
...
int recvall(int socket_recibir, char *buffer, int buffer_size){
	int nRet;
	while((nRet = recv(socket_recibir, buffer, buffer_size, MSG_WAITALL))>0){
